...
async def save_data(
    ...
    merged_df: pd.DataFrame = _data_preprocess(
        metrics_path,
        settings_path
    )
...
def _data_preprocess( 
    ...
    merged_df['timestamp_x'] =  pd.to_datetime(merged_df['timestamp_x'], format='%Y-%m-%d %H:%M:%S.%f' ,errors='coerce')
    merged_df['timestamp_y'] = pd.to_datetime(merged_df['timestamp_y'], format='%Y-%m-%d %H:%M:%S.%f', errors='coerce')    
    merged_df['timestamp'] = (merged_df['timestamp_x'] - merged_df['timestamp_y']).astype(np.int64) // 10**9
